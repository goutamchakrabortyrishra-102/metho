            ...
            <Section title="WhatsApp Cloud API" subtitle="Configure WhatsApp Business / Cloud API lead intake without touching the working Meta integration." icon={SettingsIcon} badge="Admin">
              ...
              <div className="md:col-span-2 border-t border-border pt-4">
                ...
                <div className="mt-2 grid gap-3 md:grid-cols-2">
                  ...
                  {[ ["registration_welcome_message", "Registration welcome message"], ["registration_url", "Registration URL"], ["registration_help_prompt", "Registration help prompt"]].map(([key, label]) => <div key={key}><Label>{label}</Label><textarea value={whatsappForm[key] || ""} onChange={(e) => updateWhatsappField(key)(e.target.value)} placeholder={`Set ${label.toLowerCase()}`} className="mt-1.5 min-h-20 w-full rounded-md border border-input bg-white px-3 py-2 text-sm" /></div>)}
                  {[ ["registration_role_question", "Registration role selection question"], ["member_registration_url", "Member registration URL"], ["member_registration_reply", "Member auto-reply"], ["member_registration_keywords", "Member reply keywords (comma-separated)"], ["partner_registration_url", "Partner registration URL"], ["partner_registration_reply", "Partner auto-reply"], ["partner_registration_keywords", "Partner reply keywords (comma-separated)"], ["rider_registration_url", "Rider registration URL"], ["rider_registration_reply", "Rider auto-reply"], ["rider_registration_keywords", "Rider reply keywords (comma-separated)"]].map(([key, label]) => <div key={key}><Label>{label}</Label><textarea value={whatsappForm[key] || ""} onChange={(e) => updateWhatsappField(key)(e.target.value)} placeholder={`Set ${label.toLowerCase()}`} className="mt-1.5 min-h-20 w-full rounded-md border border-input bg-white px-3 py-2 text-sm" /></div>)}
                </div>
              </div>
