...
export default function SettingsPage() {
  ...
  const saveWhatsapp = async () => {
    ...
    try {
      ...
      ["default_auto_reply", "default_auto_reply_image_url", "default_auto_reply_mode", "customer_auto_reply", "customer_auto_reply_image_url", "customer_auto_reply_mode", "member_auto_reply", "member_auto_reply_image_url", "member_auto_reply_mode", "partner_auto_reply", "partner_auto_reply_image_url", "partner_auto_reply_mode", "invoice_template", "order_template", "registration_welcome_message", "registration_welcome_message_image_url", "registration_welcome_message_mode", "registration_url", "registration_help_prompt", "registration_role_question", "member_registration_url", "member_registration_reply", "member_registration_reply_image_url", "member_registration_reply_mode", "member_registration_keywords", "partner_registration_url", "partner_registration_reply", "partner_registration_reply_image_url", "partner_registration_reply_mode", "partner_registration_keywords", "rider_registration_url", "rider_registration_reply", "rider_registration_reply_image_url", "rider_registration_reply_mode", "rider_registration_keywords"].forEach((key) => {
        ...
        <fieldset disabled={readOnly} className="space-y-6">
            ...
            <Section title="WhatsApp Cloud API" subtitle="Configure WhatsApp Business / Cloud API lead intake without touching the working Meta integration." icon={SettingsIcon} badge="Admin">
              ...
              <div className="md:col-span-2 border-t border-border pt-4">
                ...
                <div className="mt-2 grid gap-3 md:grid-cols-2">
                  ...
                  {[ ["registration_welcome_message", "Welcome message"], ["registration_url", "General registration URL"], ["registration_help_prompt", "Registration help prompt"], ["registration_role_question", "Member / Partner / Rider selection question"]].map(([key, label]) => <div key={key} className={key === "registration_role_question" ? "md:col-span-2" : ""}><Label>{label}</Label><textarea value={whatsappForm[key] || ""} onChange={(e) => updateWhatsappField(key)(e.target.value)} placeholder={`Set ${label.toLowerCase()}`} className="mt-1.5 min-h-20 w-full rounded-md border border-input bg-white px-3 py-2 text-sm" />{key === "registration_welcome_message" ? <div className="mt-2 flex flex-wrap gap-2"><select value={whatsappForm.registration_welcome_message_mode || "text"} onChange={(e) => updateWhatsappField("registration_welcome_message_mode")(e.target.value)} className="rounded border border-input px-2 py-1 text-xs"><option value="text">Send text only</option><option value="image">Send poster only</option></select><label className="inline-flex cursor-pointer items-center rounded border border-emerald-300 px-2 py-1 text-xs font-semibold text-emerald-800"><Upload className="mr-1 h-3.5 w-3.5" />Attach poster<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => uploadPresetPoster("registration_welcome_message", e)} disabled={whatsappPosterBusy} /></label>{whatsappForm.registration_welcome_message_image_url ? <Button type="button" size="sm" variant="outline" onClick={() => deletePresetPoster("registration_welcome_message")} disabled={whatsappPosterBusy}>Delete poster</Button> : null}</div> : null}</div>)}
                  <div className="md:col-span-2 rounded border border-emerald-200 bg-emerald-50 p-3"><Label>Member / Partner / Rider poster mode</Label><p className="mb-2 text-xs text-emerald-800">Choose text only or poster only for each registration reply.</p><div className="grid gap-2 md:grid-cols-3">{[["member", "Member"], ["partner", "Partner"], ["rider", "Rider"]].map(([role, label]) => <label key={role} className="text-xs font-semibold text-emerald-950">{label}<select value={whatsappForm[`${role}_registration_reply_mode`] || "text"} onChange={(e) => updateWhatsappField(`${role}_registration_reply_mode`)(e.target.value)} className="mt-1 w-full rounded border border-input bg-white px-2 py-2 font-normal"><option value="text">Text only</option><option value="image">Poster only</option></select></label>)}</div></div>
                  {[ ["Member", "member"], ["Partner", "partner"], ["Rider", "rider"] ].map(([title, role]) => <div key={role} className="md:col-span-2 border-t border-border pt-3"><p className="text-sm font-semibold text-emerald-950">{title} Registration</p><div className="mt-2 grid gap-3 md:grid-cols-2"><div><Label>{title} registration URL</Label><textarea value={whatsappForm[`${role}_registration_url`] || ""} onChange={(e) => updateWhatsappField(`${role}_registration_url`)(e.target.value)} placeholder="https://..." className="mt-1.5 min-h-20 w-full rounded-md border border-input px-3 py-2 text-sm" /></div><div><Label>{title} reply keywords (comma-separated)</Label><textarea value={whatsappForm[`${role}_registration_keywords`] || ""} onChange={(e) => updateWhatsappField(`${role}_registration_keywords`)(e.target.value)} placeholder="1, member" className="mt-1.5 min-h-20 w-full rounded-md border border-input px-3 py-2 text-sm" /></div><div className="md:col-span-2"><Label>{title} auto-reply</Label><textarea value={whatsappForm[`${role}_registration_reply`] || ""} onChange={(e) => updateWhatsappField(`${role}_registration_reply`)(e.target.value)} placeholder={`Set ${title.toLowerCase()} auto-reply`} className="mt-1.5 min-h-20 w-full rounded-md border border-input px-3 py-2 text-sm" /><div className="mt-2 flex gap-2"><label className="inline-flex cursor-pointer items-center rounded border border-emerald-300 px-2 py-1 text-xs font-semibold text-emerald-800"><Upload className="mr-1 h-3.5 w-3.5" />Attach poster<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => uploadPresetPoster(`${role}_registration_reply`, e)} disabled={whatsappPosterBusy} /></label>{whatsappForm[`${role}_registration_reply_image_url`] ? <Button type="button" size="sm" variant="outline" onClick={() => deletePresetPoster(`${role}_registration_reply`)} disabled={whatsappPosterBusy}>Delete poster</Button> : null}</div></div></div></div>)}
                </div>
              </div>
          ...
          {!readOnly && shippingForm ? (
            <Section title="Shipping Provider" subtitle="Configure courier credentials here. Existing orders stay unchanged until a shipment action is connected and enabled." icon={SettingsIcon} badge="Admin">
              ...
              <Field label="Provider" value={shippingForm.provider || ""} onChange={updateShippingField("provider")} type="text" placeholder="ithink" />
