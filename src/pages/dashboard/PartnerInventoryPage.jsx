...
export default function PartnerInventoryPage() {
  ...
  return <div className="partner-inventory-page space-y-5" data-testid="partner-inventory-page">
    ...
    <div className="overflow-x-auto rounded-xl border border-border bg-white"><table className="min-w-full text-sm"><thead className="bg-emerald-900 text-white"><tr>{config.fields.map((field) => <th key={field} className="px-3 py-3 text-left whitespace-nowrap">{field}</th>)}<th className="px-3 py-3 text-left print:hidden">Actions</th></tr></thead><tbody className="divide-y divide-border">{rows.map((item) => <tr key={item.item_id || item.id}><td className="px-3 py-3 font-semibold">{item.name || "Unnamed listing"}</td><td className="px-3 py-3">{item.category || item.service_sector || item.property_type || "-"}</td><td className="px-3 py-3">{sector === "product" ? `${item.current_stock ?? item.stock ?? 0}` : `INR ${Number(item.price || 0).toLocaleString("en-IN")}`}</td><td className="px-3 py-3">{sector === "product" ? Math.max(0, Number(item.current_stock ?? item.stock ?? 0)) : item.availability || item.vehicle_status || item.delivery_status || item.property_status || "-"}</td><td className="px-3 py-3"><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-800">{statusFor(sector, item)}</span></td><td className="px-3 py-3 print:hidden"><Link to={`/partner?tab=${sector === "product" ? "products" : sector === "transport" ? "transport" : sector === "courier" ? "delivery-partner" : sector === "property" ? "property-buy-sell" : "services"}`} className="inline-flex items-center text-emerald-800 hover:underline"><Pencil className="mr-1 h-3.5 w-3.5" /> Edit</Link></td></tr>)}{!loading && !error && rows.length === 0 ? <tr><td colSpan={config.fields.length + 1} className="px-4 py-10 text-center text-slate-500">No inventory found.</td></tr> : null}</tbody></table></div>
