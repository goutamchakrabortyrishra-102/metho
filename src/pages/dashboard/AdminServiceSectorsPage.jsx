...
export default function AdminServiceSectorsPage({ deliveryVertical = "", methoDeliveryOnly = false }) {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [partnerResponse, orderResponse, deliveryResponse, driverResponse] = await Promise.all([
        ...
        api.get("/admin/delivery/bookings", { params: deliveryVertical ? { vertical: deliveryVertical } : undefined }),
  ...
  const title = deliveryVertical === "metho_delivery" ? "METHO Delivery Control" : "Courier / Logistics Control";
  const subtitle = deliveryVertical === "metho_delivery"
    ? "Company-controlled delivery operations from listing and payment to agent assignment and live delivery."
    : "Monitor courier, logistics, parcel and delivery partners from listing to live delivery.";
