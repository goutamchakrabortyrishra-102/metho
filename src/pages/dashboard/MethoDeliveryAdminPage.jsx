...
export default function MethoDeliveryAdminPage() {
  const [settings, setSettings] = useState({ metho_delivery_smart_cycle_percent: 0, metho_delivery_reward_pool_percent: 0 });
  ...
  const save = async (event) => {
    ...
    if (![smartCycle, rewardPool].every((value) => Number.isFinite(value) && value >= 0 && value <= 100)) {
      toast.error("Both percentages must be between 0 and 100");
      return;
    }
    setSaving(true);
    try {
      await api.put("/settings", { metho_delivery_smart_cycle_percent: smartCycle, metho_delivery_reward_pool_percent: rewardPool });
