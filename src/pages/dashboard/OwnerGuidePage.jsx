    ...
    <div className="space-y-6" data-testid="owner-guide-page">
      ...
      <section className="border border-emerald-200 bg-emerald-50 p-5" data-testid="owner-guide-live-work">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Link to="/app/crm/whatsapp" className="border border-amber-200 bg-white p-3"><Users className="h-4 w-4 text-amber-700" /><p className="mt-2 text-xs text-slate-500">Executive handoff</p><p className="text-2xl font-bold text-emerald-950">{liveData?.overview?.whatsapp_human_handoffs ?? "-"}</p><p className="mt-1 text-xs text-slate-600">যারা মানুষের সাথে কথা চাইছে</p></Link>
          <Link to="/app/crm/whatsapp" className="border border-red-200 bg-white p-3"><Clock3 className="h-4 w-4 text-red-700" /><p className="mt-2 text-xs text-slate-500">Reminder no response</p><p className="text-2xl font-bold text-red-700">{liveData?.overview?.whatsapp_no_response_after_reminders ?? "-"}</p><p className="mt-1 text-xs text-slate-600">দুই reminder-এর পরও reply নেই</p></Link>
          <Link to="/app/orders" className="border border-amber-200 bg-white p-3"><Wallet className="h-4 w-4 text-amber-700" /><p className="mt-2 text-xs text-slate-500">Order / invoice action</p><p className="text-2xl font-bold text-emerald-950">{(liveData?.overview?.pending_orders || 0) + (liveData?.overview?.paid_orders_without_invoice || 0)}</p><p className="mt-1 text-xs text-slate-600">payment, approval, invoice দেখুন</p></Link>
        </div>
