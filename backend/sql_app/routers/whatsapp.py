...
def update_whatsapp_settings(payload: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    next_config = {
        ...
        "registration_welcome_message_image_url": str(data.get("registration_welcome_message_image_url", current.get("registration_welcome_message_image_url", "")) or "").strip(),
